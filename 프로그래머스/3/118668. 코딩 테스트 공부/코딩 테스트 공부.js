...
function solution(alp, cop, problems) {
    ...
    let powerTable = new Array(maxAlp + 1).fill(null).map(() => new Array(maxCop + 1).fill(Infinity));
    ...
    for(let a = alp; a <= maxAlp; a++) {
        for(let c = cop; c <= maxCop; c++) {
            ...
            if (a + 1 <= maxAlp)
                powerTable[a + 1][c] = Math.min(powerTable[a][c] + 1, powerTable[a + 1][c]);
            if (c + 1 <= maxCop)
                powerTable[a][c + 1] = Math.min(powerTable[a][c] + 1, powerTable[a][c + 1]);
            ...
            for(let tmp = 0; tmp < problems.length; tmp++) {
                ...
                if(alp_req <= a && cop_req <= c){
                    ...
                    const targetCop = c + cop_rwd >= maxCop ? maxCop : c + cop_rwd;
                        ...
                        = Math.min(powerTable[targetAlp][targetCop], powerTable[a][c] + cost)
                }       
            }
        }
    }
